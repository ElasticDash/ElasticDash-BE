...
## API Endpoint Formatting Rule

All API endpoint functions must use the following import and response pattern:
...
router.post('/example', async (req, res) => {
  // Identify and export each requried attribute from req, including body, params, query
  // Avoid using req directly in the logic
  // Avoid using query or parameters in POST requests
  const { attribute1, attribute2 } = req.body;
  try {
    // Avoid sending a JSON object directly in response, use separated attributes instead
    functionLogicHere(attribute1, attribute2);
    // ...your logic
...
This ensures all endpoints have consistent error and response handling, matching the style in services/plan.js.

## Core Workflow: Plan → Confirm → Execute

You are not allowed to create any commit or push.
...
## Files To Be Checked
- `path/to/file1.ts` - [what to read]
- `path/to/file2.tsx` - [what to read]
...
- ✅ If any `import` or `require` statements are needed, add them at the top of the file
...
- Modify the function inside the controller folder for new db action, do not put db actions in services folder.