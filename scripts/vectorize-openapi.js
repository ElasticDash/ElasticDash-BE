    ...
    Object.entries(paths).forEach(([apiPath, methods]) => {
      Object.entries(methods).forEach(([method, op]) => {
        ...
        if (Array.isArray(op.parameters)) {
          params = op.parameters.map((p) => `${p.name} (${p.in}): ${p.schema ? p.schema.type : ''}`).join('; ');
        }
        // Request body
        let requestBody = '';
...
const saveInitialUnvectorizedData = async (apiChunks, sqlChunks) => {
...
const processVectorization = async (apiChunks, sqlChunks) => {
  ...
  let apiVectorizedData = [];
  let tableVectorizedData = [];
