import { cosineSimilarity } from '../chat/similarity';
import fs from 'fs';
...
const vectorizedDataPath = path.join(process.cwd(), 'data/vectorized-data/vectorized-data.json');
const vectorizedDataTablePath = path.join(process.cwd(), 'data/vectorized-data/table/vectorized-data.json');
const vectorizedDataApiPath = path.join(process.cwd(), 'data/vectorized-data/api/vectorized-data.json');
...
function findTopKSimilarApi(queryEmbedding, topK = 3, context) {
...
function findTopKSimilarTable(queryEmbedding, topK = 3, context) {
...
export async function getTopKResults(allMatchedApis, topK) {
  ...
  const hasSqlEntry = topKResults.some((item) => item.id === 'sql-query');
  if (!hasSqlEntry && sqlEntry) {
    topKResults.push(sqlEntry);
  }

  topKResults = topKResults.map((item) => {
    const topK = {
      id: item.id,
...
export async function fetchPromptFile(fileName) {
  try {
...
export function detectEntityName(refinedQuery) {
...
export function replaceInObject(obj, search, replacement) {
...
export function sanitizePlannerResponse(response) {
  try {
