...
import { dynamicApiRequest } from './apiService';
import { findApiParameters } from './apiSchemaLoader';
// import { clarifyAndRefineUserInput, handleQueryConceptsAndNeeds } from '@/utils/queryRefinement';
import { sendToPlanner } from './planner';
// import { getAllMatchedApis, getTopKResults } from '@/services/chatPlannerService';
...
function detectEntityName(refinedQuery) {
...
async function runPlannerWithInputs({
  ...
  const hasSqlCandidate = topKResults.some((item) => item.id && typeof item.id === 'string' && (item.id.startsWith('table-') || item.id === 'sql-query'));
  ...
  if (referenceTask && referenceTask.steps && Array.isArray(referenceTask.steps) && referenceTask.steps.length > 0) {
    ...
    const executionPlan = referenceTask.steps.map((step) => {
    ...
    try {
      ...
      try {
        ...
        if (planObj && planObj.execution_plan && Array.isArray(planObj.execution_plan)) {
          planObj.execution_plan = planObj.execution_plan.map((step) => {
            if (
              step.api &&
        ...
        if (planObj && planObj.selected_tools_spec && Array.isArray(planObj.selected_tools_spec)) {
          planObj.selected_tools_spec = planObj.selected_tools_spec.map((tool) => {
            if (
              tool.endpoint &&
              ...
              if (Array.isArray(tool.derivations)) {
                tool.derivations = tool.derivations.filter((d) => !d.toLowerCase().includes('user_id'));
              }
            }
            return tool;
          });
...
Output:`;
    ...
    const shortlistedTables = topKResults.filter((table) => 
      tableSelection.selected_tables.some((selectedId) => 
        table.id === selectedId || table.id.includes(selectedId) || selectedId.includes(table.id)
      )
    );
    
    console.log('📊 Shortlisted Tables:', shortlistedTables.map((t) => t.id));
...
SQL:`;
    ...
    if (selectMatch) {
      ...
      if (aliases.size > 0) {
        sqlText = sqlText.replace(/HAVING\s+(.+?)(?=\s+(?:ORDER|LIMIT|;|$))/gi, (havingClause) => {
...
function containsPlaceholderReference(obj) {
...
async function resolvePlaceholders(
  ...
  const apiKey_local = process.env.OPENAI_API_KEY;
  if (!apiKey_local) {
...
export async function postDuplicatedChat(requestBody, myId, userToken) {
  ...
  try {
    ...
    console.error('Error in postDuplicatedChat:', error);
...
Parameters: ${JSON.stringify(apiSchema.parameters || {}, null, 2)}`;
    ...
    if (availableApis && availableApis.length > 0) {
      ...
      const apiSummaries = availableApis.slice(0, 10).map((api) => {
        try {
...
// Use the actual data from the API responses to provide specific, accurate information.`;
    ...
    let systemPrompt = 'You are an expert assistant dedicated to synthesizing information from API responses to address user queries. Your task is to deliver answers that are clear, concise, and well-structured, drawing directly from the data provided by the APIs. Ensure your responses are specific and accurate, always grounding your explanations in the actual API results.';
...
async function executeIterativePlanner(
  ...
  let intentType = matchedApis[0]?.id.startsWith('semantic') ? 'FETCH' : 'MODIFY';
  ...
  while (planIteration < 20) { // Max 20 planning cycles (separate from API call limit)
    ...
    try {
      ...
      while (actionablePlan.execution_plan?.length > 0) {
        ...
        if (step.api && step.api.path && step.api.method) {
            ...
            if (previousStepResult && previousStepResult.response) {
              ...
              if (Array.isArray(results) && results.length > 1) {
                ...
                if (pathParamMatches && pathParamMatches.length > 0) {
                  ...
                  stepsToExecute = results.map((result, index) => {
          ...
          for (const stepToExecute of stepsToExecute) {
              ...
              if (previousStepResult && previousStepResult.response) {
                ...
                if (results && Array.isArray(results)) {
                  ...
                  if (Array.isArray(results) && results.length > 0) {
                    ...
                    const populateEmptyArrays = (obj, path = '') => {
                      for (const key in obj) {
                        ...
                        if (Array.isArray(obj[key]) && obj[key].length === 0) {
                          ...
                          if (key.toLowerCase().includes('type')) {
                            extractedIds = results.slice(0, numIds).map((item) =>
                    ...
                    const populateSingleIds = (obj, path = '') => {
                      for (const key in obj) {
                        const fullPath = path ? `${path}.${key}` : key;
                    ...
                    if (pathParamMatches && pathParamMatches.length > 0) {
                      ...
                      pathParamMatches.forEach((placeholder) => {
            ...
            function getApiCallKey(path, method, params, body) {
            ...
            const flatUsefulDataMap = requestContext.flatUsefulDataMap;
      ...
      if (intentType === 'MODIFY' && !hasApiError) {
        ...
        const sanitizeForValidation = (obj) => {
          const seen = new WeakSet();
          return JSON.parse(JSON.stringify(obj, (key, value) => {
            if (typeof value === 'object' && value !== null) {
...
Please generate the next step in the plan, or indicate that no more steps are needed.`;
  ...
  const sanitizeForFinalAnswer = (obj) => {
    const seen = new WeakSet();
    return JSON.parse(JSON.stringify(obj, (key, value) => {
      if (typeof value === 'object' && value !== null) {
        if (seen.has(value)) return '[Circular]';
  ...
  const preparedResults = sanitizedAccumulatedResults.map((result) => {
    ...
    if (response && response.result) {
      ...
      if (resultData.moves && Array.isArray(resultData.moves) && resultData.moves.length > 10) {
        ...
        if (mentionedType) {
          const relevantMoves = resultData.moves.filter((move) =>
